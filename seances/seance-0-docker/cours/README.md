...
- [Séance 0 — Docker : histoire et fondamentaux (cours)](#séance-0--docker--histoire-et-fondamentaux-cours)
  ...
  - [2. Pourquoi les conteneurs sont une révolution](#2-pourquoi-les-conteneurs-sont-une-révolution)
    - [2.1. Cascade, agile, DevOps : l'accélération du cycle de développement](#21-cascade-agile-devops--lacceleration-du-cycle-de-developpement)
    ...
    - [2.4. Ce que ça change concrètement pour vous](#24-ce-que-ça-change-concretement-pour-vous)
  - [3. Terminologie Docker](#3-terminologie-docker)
  - [4. Construire une image : le Dockerfile](#4-construire-une-image--le-dockerfile)
  - [5. Lancer et gérer des conteneurs](#5-lancer-et-gerer-des-conteneurs)
  - [6. Les volumes](#6-les-volumes)
  - [7. Docker Compose et le DNS interne](#7-docker-compose-et-le-dns-interne)
  - [8. Aller plus loin](#8-aller-plus-loin)
    - [8.1. Images de base et Alpine](#81-images-de-base-et-alpine)
    - [8.2. Multi-stage builds](#82-multi-stage-builds)
    - [8.3. Registry d'images](#83-registry-dimages)
    - [8.4. Développer depuis un conteneur](#84-développer-depuis-un-conteneur)
  - [9. Synthèse et lien avec la suite du module](#9-synthèse-et-lien-avec-la-suite-du-module)
...
Ce socle vous servira dès la séance 1, quand vous conteneuriserez votre projet GearShare avec au
minimum 3 services (backend, frontend, base de données).
...
## 7. Docker Compose et le DNS interne
...
L'URL prend la forme `http://<nom_du_service>:<port>/`. C'est ce mécanisme qui permettra à votre
frontend de contacter votre backend, et à votre backend de contacter votre base de données,
uniquement en connaissant leur nom de service dans le `docker-compose.yml`.

## 8. Aller plus loin

### 8.1. Images de base et Alpine
...
### 8.4. Développer depuis un conteneur
...
## 9. Synthèse et lien avec la suite du module
...
est une pièce indépendante) et le DNS interne de Docker Compose (les services se contactent par
leur nom, pas par leur IP).

Ce socle sera mobilisé concrètement à partir de la séance 1, quand vous devrez conteneuriser
votre projet GearShare en au moins 3 services (backend, frontend, base de données PostgreSQL).
